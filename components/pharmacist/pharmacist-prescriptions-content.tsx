  ...
  const handleApprove = async () => {
    ...
    try {
      await ordersApi.updateStatus(selectedPrescription.id, {
        ...
        notes: reviewNotes || "Prescription approved"
      })
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Review and approve prescription orders</p>
        </div>
        ...
        <Card className="border-yellow-200 bg-yellow-50/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-yellow-600">
              {prescriptions.filter(p => p.status === 0).length}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-blue-200 bg-blue-50/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">
              {prescriptions.filter(p => p.status === 1 || p.status === 2).length}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-green-200 bg-green-50/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {prescriptions.filter(p => p.status === 4).length}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-red-200 bg-red-50/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {prescriptions.filter(p => p.status === 5 || p.status === 6).length}
            </div>
          </CardContent>
        </Card>
      </div>
                  ...
                  >
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                        <div className="flex items-center gap-1 text-muted-foreground">
                          ...
                          {prescription.customerName || "Unknown"}
                        </div>
                        <div className="flex items-center gap-1 text-muted-foreground">
                    ...
                    <div className="flex gap-2">
                      ...
                      {prescription.status === 0 && (
                        <Button
                          size="sm"
                          onClick={() => openReviewDialog(prescription)}
                        >
                          Review
                        </Button>
                      )}
      ...
      <Dialog open={isReviewDialogOpen} onOpenChange={setIsReviewDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Review the prescription and approve or reject the order
            </DialogDescription>
          </DialogHeader>
